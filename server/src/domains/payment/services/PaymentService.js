...
class PaymentService {
  ...
  initWechatPay() {
    try {
      if (
        appConfig.wechatPay.appId &&
        appConfig.wechatPay.mchId &&
        appConfig.wechatPay.privateKey
      ) {
        this.wechatPayInstance = new WechatPay({
          appid: appConfig.wechatPay.appId,
          mchid: appConfig.wechatPay.mchId,
          publicKey: appConfig.wechatPay.publicKey,
          privateKey: appConfig.wechatPay.privateKey,
          key: appConfig.wechatPay.key,
          serial_no: appConfig.wechatPay.serialNo,
  ...
  initAlipay() {
    try {
      if (
        appConfig.alipay.appId &&
        appConfig.alipay.privateKey
      ) {
        this.alipayInstance = new AlipaySdk({
          appId: appConfig.alipay.appId,
          privateKey: appConfig.alipay.privateKey,
          signType: 'RSA2',
          gateway: appConfig.alipay.isDev ? 'https://openapi.alipaydev.com/gateway.do' : 'https://openapi.alipay.com/gateway.do',
          alipayPublicKey: appConfig.alipay.publicKey,
          timeout: 5000
        });
      } else {
  ...
  async createAlipayPayment(options) {
    try {
      ...
      formData.setReturnUrl(appConfig.alipay.returnUrl);
